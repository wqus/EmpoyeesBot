...
def lessons(items):
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{('🔒' if p.status.value == 'locked' else '✅' if p.status.value == 'passed' else '▶️')} {p.lesson.title}", callback_data=f'lesson:{p.lesson_id}')] for p in items])
